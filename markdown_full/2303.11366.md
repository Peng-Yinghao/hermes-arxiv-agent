...
authors: ["Noah Shinn", "Northeastern University", "Federico Cassano", "Edward Berman", "Ashwin Gopinath", "Massachusetts Institute of Technology", "Karthik Narasimhan", "Princeton University", "Shunyu Yao"]
...
Abstract Large language models (LLMs) have been increasingly used to interact with external
...
Figure: Figure 1: Reflexion works on decision-making [4.1](#S4.SS1), programming [4.3](#S4.SS3), and reasoning [4.2](#S4.SS2) tasks.
...
Figure: Algorithm 1 Reinforcement via self-reflection
Refer to caption: x2.png

We develop a modular formulation for Reflexion, utilizing three distinct models:
an Actor, denoted as $M_{a}$, which generates text and actions;
an Evaluator model, represented by $M_{e}$, that scores the outputs produced
by $M_{a}$; and a Self-Reflection model, denoted as $M_{sr}$,
...
or generation, $a_{t}$, from the current policy $\pi_{\theta}$ at time $t$, receive
an observation from the environment $o_{t}$. We explore various Actor models,
...
action $a_{i}$ led to subsequent incorrect actions $a_{i+1}$ and $a_{i+2}$. The
agent can then verbally state that it should have taken a different action,
$a_{i}^{\prime}$, which would have resulted in $a_{i+1}^{\prime}$ and $a_{i+2}^{\prime}$, and store
...
choosing action $a_{i}^{\prime}$. This iterative process of trial, error, self-reflection,
...
Reflexion is formalized as an iterative optimization process in [1](#alg1).
In the first trial, the Actor produces a trajectory $\tau_{0}$ by
interacting with the environment. The Evaluator then produces a score $r_{0}$
which is computed as $r_{t}=M_{e}(\tau_{0})$. $r_{t}$ is only a scalar
reward for trial $t$ that improves as task-specific performance increases.
After the first trial, to amplify $r_{0}$ to a feedback form that can be used
for improvement by an LLM, the Self-Reflection
model analyzes the set of $\{\tau_{0},r_{0}\}$ to produce a summary
$sr_{0}$ which is stored in the memory *mem*. $sr_{t}$ is a verbal
experience feedback for trial $t$. The Actor, Evaluator, and Self-Reflection
models work together through trials in a loop until the Evaluator deems $\tau_{t}$ to
be correct. As mentioned in [3](#S3.SS0.SSS0.Px4), the memory component of
Reflexion is crucial to its effectiveness. After each trial $t$, $sr_{t}$, is appended
*mem*. In practice, we bound *mem* by a maximum number of stored experiences, $\Omega$
...
Figure: Figure 3: (a) AlfWorld performance across 134 tasks showing cumulative proportions of solved tasks using self-evaluation techniques of (Heuristic) and (GPT) for binary classification. (b) Classification of AlfWorld trajectories by reason of failure.
...
In [3](#S4.F3), the learning curve suggests that
...
next 11 trials to a near-perfect performance. On the other hand, [3](#S4.F3)
...
+ Chain-of-Thought (CoT) (Wei et al.,, 2022) for step-by-step $Q\rightarrow A$ and
...
the decision-making setup [4.1](#S4.SS1) in AlfWorld with a
memory size of 3 experiences.

##### Results

Figure: Figure 4: Chain-of-Thought (CoT) and ReAct. Reflexion improves search, information retrieval, and reasoning capabilities on 100 HotPotQA questions. (a) Reflexion ReAct vs Reflexion CoT (b) Reflexion CoT (GT) for reasoning only (c) Reflexion vs episodic memory ablation.
...
step for reasoning using CoT (GT) as the baseline approach [4](#S4.F4).
Recall that CoT (GT)
...
explanation using language written in the first person. [4](#S4.F4)
...
test suite $T$, denoted as $\{t_{0},t_{1},\dots,t_{n}\}$. We set $n$ to a maximum
...
| --- | --- | --- | --- |
| HumanEval (PY) | 65.8 (CodeT (Chen et al.,, 2022) + GPT-3.5) | 80.1 (GPT-4) | 91.0 |
...
on all benchmarks for Python and Rust except for MBPP Python [1](#S4.T1).
...
report an invalid submission. In [2](#S4.T2), various conditions
...
Python. In [2](#S4.T2), we observe a notable discrepancy between
...
current implementations. [3](#S4.T3) shows an inferior
...
[6](#A2.F6). Further, the agent does not generate helpful,
...
Figure: Figure 6: Reflexion vs React performance on WebShop across 100 customer shopping requests. ReAct + Reflexion fails to significantly outperform ReAct.
...
See the full implementation at [https://github.com/noahshinn024/reflexion](https://github.com/noahshinn024/reflexion).
...
Figure: Figure 7: Two HotPotQA trials within the same environment and task. The Reflexion + ReAct agent uses self-reflection to determine a better search method for the next trial.
